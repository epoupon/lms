...
LMS.mediaplayer = (function () {
    ...
    var _setReplayGain = function (replayGain) {
        _gainNode.gain.value = Math.pow(
            10,
            (_settings.replayGain.preAmpGain + replayGain) / 20
        );
    };
    ...
    var init = function (root, defaultSettings) {
        ...
        _elems.transcodingActive = document.getElementById(
            "lms-transcoding-active"
        );
        ...
        _elems.audio.addEventListener("timeupdate", function () {
            _elems.progress.style.width =
                "" +
                ((_offset + _elems.audio.currentTime) / _duration) * 100 +
                "%";
            _elems.curtime.innerHTML = _durationToString(
                _offset + _elems.audio.currentTime
            );
        });
    ...
    var loadTrack = function (params, autoplay) {
        ...
        _audioTranscodeSrc =
            params.transcodeResource +
            "&bitrate=" +
            _settings.transcode.bitrate +
            "&format=" +
            _settings.transcode.format;
        ...
        // ! order is important
        if (
            _settings.transcode.mode == TranscodeMode.Never ||
            _settings.transcode.mode == TranscodeMode.IfFormatNotSupported
        ) {
            _addAudioSource(_audioNativeSrc);
        }
        if (
            _settings.transcode.mode == TranscodeMode.Always ||
            _settings.transcode.mode == TranscodeMode.IfFormatNotSupported
        ) {
        ...
        if (autoplay && _audioCtx.state == "running") _playTrack();
