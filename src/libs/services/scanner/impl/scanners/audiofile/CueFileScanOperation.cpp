...
namespace lms::scanner
{
    namespace
    {
        ...
        void updateReleaseIfNeeded(db::Session& session, db::Release::pointer release, const Release& releaseInfo)
        {
        ...
        bool isReleaseMatching(const db::Release::pointer& candidateRelease, const Release& releaseInfo)
        {
        ...
        db::Release::pointer getOrCreateRelease(db::Session& session, const Release& releaseInfo, const db::Directory::pointer& currentDirectory)
        {
    ...
    namespace cue {
        ...
        struct ParsedCueTrack
        {
            ...
            std::unique_ptr<audio::AudioProperties> audioProperties;
            std::unique_ptr<Track> parsedTrack;
        };
    }

    CueFileScanOperation::CueFileScanOperation(FileToScan&& fileToScan, db::IDb& db, const ScannerSettings& settings, const AudioFileInfoParserSet& audioFileInfoParserSet, const TrackMetadataParser& parser)
    ...
    void CueFileScanOperation::scan()
    {
        ...
        for (auto& track : _parsedFile) {
            ...
            try {
                const auto audioFileInfo{ _audioFileInfoParserSet.taglibParser->parse(track.baseFile, options) };
                track.audioProperties = std::make_unique<audio::AudioProperties>(*audioFileInfo->getAudioProperties());
                track.parsedTrack = std::make_unique<Track>(_metadataParser.parseTrackMetaData(*audioFileInfo->getTagReader()));
                // TODO: fillMissingMbids(track.parsedTrack);
            } catch (...) {
    ...
    CueFileScanOperation::OperationResult CueFileScanOperation::processResult()
    {
        ...
        for (const auto& input : _parsedFile) {
            ...
            if (input.audioProperties->duration == std::chrono::milliseconds::zero()) {
            ...
            const auto end   = input.endFrame == cue::Frames::zero()
                             ? input.audioProperties->duration
            ...
            // Audio properties
            track.modify()->setBitrate(input.audioProperties->bitrate);
            track.modify()->setBitsPerSample(input.audioProperties->bitsPerSample);
            track.modify()->setChannelCount(input.audioProperties->channelCount);
            track.modify()->setSampleRate(input.audioProperties->sampleRate);
            ...
            rel.artists           = { Artist(input.performer) };
            auto release{ getOrCreateRelease(dbSession, rel, directory) };
            assert(release);
            track.modify()->setRelease(release);
            track.modify()->setMedium(getOrCreateMedium(dbSession, Medium{ .name = input.discTitle, .release = rel }, release));
            }
            ...
            auto artist = helpers::getOrCreateArtistByName(dbSession, Artist(input.performer), allowFallback);
