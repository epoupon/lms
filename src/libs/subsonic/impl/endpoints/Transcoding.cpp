...
namespace lms::api::subsonic
{
    ...
    Response handleGetTranscodeDecision(RequestContext& context)
    {
        ...
        try
        {
            auto audioFile{ av::parseAudioFile(track->getAbsoluteFilePath()) };
            const auto audioStreamInfo{ audioFile->getBestStreamInfo() };
            if (!audioStreamInfo)
                throw InternalErrorGenericError{ "Cannot analyze best audio stream" };

            Response response{ Response::createOkResponse(context.getServerProtocolVersion()) };
            Response::Node& transcodeNode{ response.createNode("transcodeDecision") };
            transcodeNode.addChild("sourceStream", createStreamDetails(audioFile->getContainerInfo(), *audioStreamInfo));

            const details::TranscodeDecisionResult transcodeDecision{ details::computeTranscodeDecision(clientInfo, audioFile->getContainerInfo(), *audioStreamInfo) };
