...
#include "PulseAudioException.hpp"

static
pa_sample_spec
constructSampleSpec(AudioOutput::Format /*format*/, AudioOutput::SampleRate sampleRate, std::size_t nbChannels)
{
	return pa_sample_spec
	{
		.format = PA_SAMPLE_S16LE, // TODO
		.rate = static_cast<uint32_t>(sampleRate),
		.channels = static_cast<uint8_t>(nbChannels),
	};
}
